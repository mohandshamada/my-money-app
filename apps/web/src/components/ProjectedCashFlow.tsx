...
export function ProjectedCashFlow() {
  ...
  const projection = useMemo(() => {
    ...
    let currentBalance = transactions.reduce((acc, t) => {
      ...
      return (t.isExpense || t.is_expense) ? acc - amount : acc + amount
    }, 0)
  ...
  return (
    <div className="card">
      ...
      {/* Simple Chart */}
      <div className="relative h-40 sm:h-48 bg-gray-50 dark:bg-gray-800 rounded-lg p-4 mb-4">
        ...
        {/* Labels */}
        <div className="absolute top-2 right-2 text-xs text-gray-700 dark:text-gray-300 font-medium">
          ${Number(chartData?.max || 0).toFixed(0)}
        </div>
        <div className="absolute bottom-2 right-2 text-xs text-gray-700 dark:text-gray-300 font-medium">
          ${Number(chartData?.min || 0).toFixed(0)}
        </div>
      </div>
...
// Helper functions
function getRecurringAmount(transactions: any[], isExpenseFlag: boolean): number {
  const relevant = transactions.filter(t => !!(t.isExpense || t.is_expense) === isExpenseFlag)
...
function getDayExpenses(date: Date, recurringExpenses: number, transactions: any[]): number {
  ...
  const dueToday = transactions.filter(t => 
    (t.isExpense || t.is_expense) && t.date === dateStr
  )
  
  if (dueToday.length > 0) {
    return dueToday.reduce((sum, t) => sum + (Number(t.amount) || 0), 0)
  }
