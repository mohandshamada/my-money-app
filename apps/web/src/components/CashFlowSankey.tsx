...
export function CashFlowSankey() {
  ...
  return (
    <div className="card">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 sm:gap-4">
        ...
        <div className="flex flex-col items-center justify-center">
          <div className="text-center mb-4">
            <p className="text-2xl font-bold text-green-600">${Number(flowData.totalIncome || 0).toFixed(0)}</p>
          ...
          <div className="text-center mt-4">
            <p className={`text-2xl font-bold ${flowData.savings >= 0 ? 'text-green-600' : 'text-red-600'}`}>
              {flowData.savings >= 0 ? '+' : ''}{Number(flowData.savings || 0).toFixed(0)}
            </p>
      ...
      {/* Summary */}
      <div className="mt-6 pt-4 border-t grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4 text-center">
        <div>
          <p className="text-lg font-semibold text-green-600">${Number(flowData.totalIncome || 0).toFixed(0)}</p>
          <p className="text-xs text-gray-500">In</p>
        </div>
        <div>
          <p className="text-lg font-semibold text-red-600">${Number(flowData.totalExpenses || 0).toFixed(0)}</p>
          <p className="text-xs text-gray-500">Out</p>
        </div>
        <div>
          <p className={`text-lg font-semibold ${flowData.savings >= 0 ? 'text-blue-600' : 'text-red-600'}`}>
            ${Math.abs(flowData.savings || 0).toFixed(0)}
          </p>
