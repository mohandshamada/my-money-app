...
const EXCHANGERATE_API = 'https://open.er-api.com/v6/latest/'

// In-memory cache
let cachedRates: Record<string, Record<string, number>> | null = null
let lastFetchTime: number = 0
const CACHE_DURATION = 60 * 60 * 1000 // 1 hour in-memory cache

// Comprehensive fallback rates if API fails
...
// Calculate cross rates from USD base
function calculateCrossRates(usdRates: Record<string, number>): Record<string, Record<string, number>> {
  const result: Record<string, Record<string, number>> = {}
  const currencies = Object.keys(usdRates)
  
  // Add USD base
  result['USD'] = { ...usdRates, USD: 1 }
  ...
  for (const base of currencies) {
    ...
    for (const target of currencies) {
      ...
      const targetToUSD = usdRates[target]
      
      if (baseToUSD && targetToUSD) {
        result[base][target] = targetToUSD / baseToUSD
      }
    }
  }
...
export async function getExchangeRates(): Promise<Record<string, Record<string, number>>> {
  ...
  // Return cached rates if still valid (1 hour)
  if (cachedRates && (now - lastFetchTime) < CACHE_DURATION) {
    console.log('Using cached exchange rates (fetched', Math.round((now - lastFetchTime) / 1000 / 60), 'minutes ago)')
    return cachedRates
  }

  try {
    console.log('Fetching live exchange rates from internet...')
    
    // Fetch from exchangerate-api.com (free, no API key)
    const response = await axios.get(`${EXCHANGERATE_API}USD`, {
      timeout: 10000
    })

    if (response.data?.rates) {
      // Calculate cross rates for all currency pairs
      cachedRates = calculateCrossRates(response.data.rates)
      lastFetchTime = now
      
      console.log('✅ Live exchange rates fetched:', Object.keys(cachedRates).length, 'currencies')
      console.log('Sample rates - USD→EGP:', (cachedRates as Record<string, Record<string, number>>).USD?.EGP, 'USD→AUD:', (cachedRates as Record<string, Record<string, number>>).USD?.AUD)
  ...
  // Try to use localStorage cached rates
  try {
    const stored = localStorage.getItem('exchange_rates_cache')
    if (stored) {
      const parsed = JSON.parse(stored)
      if (parsed.rates && (now - parsed.timestamp) < CACHE_DURATION * 24) { // Accept up to 24 hours old from storage
        cachedRates = parsed.rates
        lastFetchTime = parsed.timestamp
        console.log('Using localStorage cached rates')
        return cachedRates as Record<string, Record<string, number>>
      }
    }
  } catch {
    // Ignore localStorage errors
  ...
  console.log('Using fallback rates (API unavailable)')
  return FALLBACK_RATES
}

/**
 * Synchronous version - returns cached or fallback rates
 * Use this when you can't await, then call getExchangeRates() to refresh
 */
export function getExchangeRatesSync(): Record<string, Record<string, number>> {
...
export function convertCurrency(
  ...
  // Check if rates is valid (not empty object) - empty {} is truthy in JS
  const hasValidRates = rates && Object.keys(rates).length > 0
  const exchangeRates = hasValidRates ? rates : (cachedRates || FALLBACK_RATES)
  ...
  if (fromCurrency === 'USD') {
    ...
    const toUSDRate = exchangeRates[fromCurrency]?.['USD']
    if (toUSDRate) {
      amountInUSD = amount * toUSDRate
    } else {
      // Fallback: use inverse rate from USD rates
      const fromUSDRate = exchangeRates['USD']?.[fromCurrency]
      if (fromUSDRate) {
        amountInUSD = amount / fromUSDRate
      } else {
        console.warn(`No rate found for ${fromCurrency} → USD`)
        return amount
      }
    }
  }
  ...
  if (toCurrency === 'USD') {
    ...
    const fromUSDRate = exchangeRates['USD']?.[toCurrency]
    if (fromUSDRate) {
      return amountInUSD * fromUSDRate
    } else {
      const toUSDRate = exchangeRates[toCurrency]?.['USD']
      if (toUSDRate) {
        return amountInUSD / toUSDRate
      } else {
        console.warn(`No rate found for USD → ${toCurrency}`)
        return amount
      }
    }
  }
